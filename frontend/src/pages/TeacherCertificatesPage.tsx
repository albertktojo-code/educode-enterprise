      ...
      {studentId ? <>
        ...
        <section className="teacher-certificates-panel teacher-certificates-history">
          ...
          {certificates.length ? <div>{certificates.map((certificate) => <article key={certificate.id}>
            <div className="teacher-certificates-card-heading"><div><small>{certificate.status === 'active' ? 'ATIVO' : 'REVOGADO'} · {formatDate(certificate.issued_at)}</small><h3>{certificate.title}</h3><p>{certificate.description || 'Sem descrição.'}</p></div><code>{certificate.verification_code}</code></div>
