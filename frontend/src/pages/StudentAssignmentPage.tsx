...
export function StudentAssignmentPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      setResult(resultData); setMessage(resultData.result_available ? 'Atividade entregue.' : 'Atividade entregue. O resultado será liberado depois.')
    } catch (error) { setMessage((error as Error).message) } finally { setBusy(false) }
  }

  if (!detail) return <section><p>{message || 'Carregando atividade...'}</p></section>
  return <section className="student-activity-page"><header className="page-header"><div><Link to="/aluno/atividades">← Minhas atividades</Link><h1>{detail.title}</h1><p>{detail.instructions}</p></div><div className="student-deadline"><span>Prazo</span><strong>{detail.due_at ? new Date(detail.due_at).toLocaleString('pt-BR') : 'Livre'}</strong></div></header>{message ? <div className="inline-message">{message}</div> : null}
